...
def one_minus_cosine_similarity(a, b):
    return 1 - cosine_similarity(a, b)


def random_score(a, b):
...
def create_filtered_qtype_ans(args):
    def create(split):
        qid2hint = {}
        filtered_qtype_ans = json.load(open(f'data/hints/{split}_filtered_qtype_ans.json'))
        for qtype_ans in filtered_qtype_ans:
            for qid in filtered_qtype_ans[qtype_ans]:
                hint = np.random.rand(36)
                qid2hint[qid] = hint
        out_file = 'data/hints/' + split + f'_{args.hint_type}.pkl'
        print(f"Dumping to {out_file}")
        pickle.dump(qid2hint, open(out_file, 'wb'))
...
def main(args):
    if args.hint_type == 'filtered_qtype_ans':
        create_filtered_qtype_ans(args)
        return
    if args.hint_type == 'one_minus_q_hints':
        similarity = one_minus_cosine_similarity
    else:
    ...
    for name in names:
        ...
        for i in tqdm(range(len(qids))):
            ...
            atts = attr_emb[attrs, :]
            exps = exp_emb[tokens]
            obj_sim = similarity(objs, exps)
            attr_sim = similarity(atts, exps)
            ...
            if args.hint_type == 'random_q_hints':
                ...
                for j in range(36):
                    ...
                    for key in object_attributes:
                        ...
                        if similarity(exp_emb[obj_token:obj_token + 1], objs[j:j + 1]) > 0.3:
                            if similarity(exp_emb[attr_token:attr_token + 1], atts[j:j + 1]) > 0.3:
                                if hint_score_attr[j] <= similarity(exp_emb[attr_token:attr_token + 1], atts[j:j + 1]):
                                    hint_score[j] = similarity(exp_emb[attr_token:attr_token + 1], atts[j:j + 1])
                                if attr not in mentioned_attrs:
                                    mentioned_attrs.append(attr)

            qid2hint[qid] = hint_score
        out_file = 'data/hints/' + name + f'_{args.hint_type}.pkl'
        print(f"Dumping to {out_file}")
        save_dir = 'data/hints'
